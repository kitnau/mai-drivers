...
static int device_open(struct inode *inode, struct file *file) {
	pr_info(DEVICE_NAME ": chrdrv device open");
	return SUCCESS;
}
static int device_release(struct inode *inode, struct file *file) {
	pr_info(DEVICE_NAME ": chrdrv device release");
	return SUCCESS;
}
        ...
        case DRIVER_BUF_IS_EMPTY:
			{
				...
				unsigned char buf_is_empty = buf_len == 0 ? 1 : 0;
				if (copy_to_user((unsigned char*)arg, &buf_is_empty, sizeof(buf_is_empty))) {
					pr_err(DEVICE_NAME ": device_ioctl: copy_to_user failed");
					return -EFAULT;
				}
			}
            break;
        default: pr_err(DEVICE_NAME ": device_ioctl: Unknown command: %d", cmd); break;
    }
...
static void __exit aut_exit(void) {
    device_destroy(dev_class, dev_no);
    class_destroy(dev_class);
    cdev_del(&dev);
    unregister_chrdev_region(dev_no, 1);
	pr_info(DEVICE_NAME ": unregistered %s\n", DEVICE_NAME);
}

module_init(aut_init);
module_exit(aut_exit);
